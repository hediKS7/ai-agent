...
async def _build_initial_state(req: ChatRequest) -> tuple[AgentState, str]:
    """Shared setup logic for both /chat and /chat/stream."""
    full_message = req.message
    if req.code_context:
        full_message = f"{req.message}\n\n```\n{req.code_context}\n```"
...
async def chat_stream(req: ChatRequest):
    state, conversation_id = await _build_initial_state(req)

    async def event_generator():
        final_text = ""
        async for step in agent_graph.astream(state):
            new_val = step.get("final_response", "")
            if new_val and new_val != final_text:
                cleaned = new_val.replace("\u2014", ",").replace("\u2013", "-")
                yield f"data: {json.dumps({'token': cleaned, 'conversation_id': conversation_id})}\n\n"
                final_text = cleaned
        ...
        async with AsyncSessionLocal() as db:
            ...
            """), {"id": str(uuid.uuid4()), "conv_id": conversation_id,
                   "content": final_text,
                   "meta": json.dumps({"agent_type": req.agent_type}),
                   "now": datetime.utcnow()})
